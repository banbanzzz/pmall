        ...
        <div class="col-sm-10 col-md-10 col-sm-offset-1 col-md-offset-1">
            <form action="cart/createOrder" method="post" id="cartForm">
                ...
                <div style="text-align: right;width: 1000px;height: 50px;font-size: 16px;color: red">
                    已选<span id="totalNum">0</span>件&nbsp;&nbsp;&nbsp;总价：<span id="totalPrice">0</span>元
                </div>
                <div class="row">
                    <div class="col-lg-4 col-md-4 col-sm-4"></div>
                    <button type="button" class="btn btn-danger btn-lg col-lg-4 col-md-4 col-sm-4" onclick="confirmPre()">确认购买
                    </button>
                </div>
            </form>
        </div>
...
<script type="text/javascript">
    ...
    $(function () {
        $.ajax({
            ...
            success: function (data) {
                if (data != null && data != "") {
                    var div = '<tr>' +
                        '<th>是否购买<input onclick="selectAll();" type="checkbox" id="allSelect" style="margin-left: 10px;"></th>' +
                        '<th>商品图</th>' +
                        '<th>商品名称</th>' +
                        '<th>商品单价</th>' +
                        '<th>商品数量</th>' +
                    ...
                    for (var i = 0; i < data.length; i++) {
                        div = div + '<tr id="goodsData' + data[i].cart_id + '">' +
                            ...
                            '</div>' +
                            '</td>' +
                            '<td><a href="goods/detail?goodsId='+ data[i].cart_goods.goods_id +'"><img src="upload/' + data[i].cart_goods.goods_img + '"/></a>' +
                            '<td><a href="goods/detail?goodsId=' + data[i].cart_goods.goods_id + '" style="text-decoration: none;">' + data[i].cart_goods.goods_name + '</a></td>' +
                            '<td><span id="singlePrice' + data[i].cart_id + '">' + data[i].cart_goods.goods_price + '</span></td>' +
                    ...
                    var empty = "<div style='width:100%;height:500px;'><div style='text-align:center;padding-top:100px;'>" +
                        "<img src='upload/cart.png'><p><h3 style='padding-bottom: 15px;'>亲，您的购物车里还没有物品哦~</h3><a href='view/index' class='btn-lg btn-danger' style='text-decoration: none;'>去逛逛</a></p>" +
                        "</div></div>";
                    $("#maincart").html(empty);
                }
            },
    ...
    function addNum(id) {
        var num = parseInt($("#num" + id).html());
        if(num == 5){
            layer.msg('每个限选5件!', {icon: 5, anim: 6, time: 2000});
            return;
        }else{
            num++;
